...
TPixel32 sourceOver(const TPixel32 &src, const TPixel32 &dst) {
...
bool validateGradientClear(const TRaster32P &readback, int width, int height,
                           const TPixel32 &outsideColor, int rectX0,
                           int rectY0, int rectX1, int rectY1) {
  readback->lock();
  for (int y = 0; y < height; ++y) {
...
bool validateAlphaBlend(const TRaster32P &readback, int width, int height,
  ...
  for (int y = 0; y < height; ++y) {
    ...
    for (int x = 0; x < width; ++x) {
      ...
      TPixel32 expected =
          inGradient ? gradientPixel(x - gradientX0, y - gradientY0) : baseColor;
      if (inOverlay) expected = sourceOver(overlayColor, expected);
...
int main() {
...

  {
    ...
    std::unique_ptr<TGraphics::RenderTarget> target =
        TGraphics::createMetalImageRenderTarget(width, height);
    if (!target) return fail("could not create alpha Metal render target");
