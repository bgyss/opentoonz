...
const double minDistance2 = 16.0;  // 4 pixels
...
public:
  ...
  void undo() const override {
    ...
    if (image->isComputedRegionAlmostOnce())
      image->findRegions();  // TODO: Remove in the future. Needed because
                             // removeStrokes is faster when not calculating
                             // regions, but then everything crashes
...
public:
  ...
  /*-- When the tool is switched during dragging, perform erase termination --*/
...
private:
  ...
  TPointD m_mousePos,  //!< Current mouse position.
      ...
      m_brushPos,      //!< Position where the brush will be painted.
      m_firstPos;      //!< First point inserted either in m_track or m_polyline
  ...
  StrokeGenerator m_track;  //!< Freehand selection generator.
  ...
  TImageP m_activeImage = nullptr;  // needed if a frame is changed mid-erase
  double m_thick;
...
void EraserTool::erase(TVectorImageP vi, const TPointD &pos) {
  ...
  TRectD inscribedSquare(pos.x - rectEdge_2, pos.y - rectEdge_2,
  ...
  while (i < vi->getStrokeCount()) {
    ...
    if (!circumscribedSquare.overlaps(
            strokeBBox)) {  // Stroke outside the square circumscribing the
                            // circle
      i++;
      it++;
      continue;
    ...
    if (inscribedSquare.contains(
            strokeBBox)) {  // Stroke entirely contained within the square
...
*/
    ...
    if (intersections.empty()) {
      // WE SHOULD ONLY CHECK ONE POINT BECAUSE IF THE STROKE DOES NOT
      // INTERSECT THE CIRCLE AND CONTAINS AT LEAST ONE POINT, IT CONTAINS ALL.
      // BUT SINCE I DON'T TRUST THE INTERSECT, I CHECK A COUPLE AND FAVOR (WITH
      // AND) NON-CONTAINMENT, AS IT'S BETTER TO DELETE ONE LESS THING THAN ONE
      // MORE
      if (tdistance2(oldStroke->getPoint(0), pos) < pointSize2 &&
          tdistance2(oldStroke->getPoint(1), pos) <
              pointSize2) {  // Stroke entirely contained within the circle
        if (*it != -1) m_undo->addOldStroke(*it, vi->getVIStroke(i));
        oneStrokeIndex[0] = i;
        vi->removeStrokes(oneStrokeIndex, true, true);
        it = m_indexes.erase(it);
    ...
    if (intersections.size() == 1) {
      if (oldStroke->isSelfLoop()) {  // Only one intersection of a closed
                                      // stroke with a circle should only happen
        // in case of grazing, so pretend nothing happened
        i++;
        it++;
        continue;
      }
      ...
      vi->notifyChangedStrokes(i);  // For now like this, but it's slow
    ...
    if (intersections.size() & 1 &&
        oldStroke->isSelfLoop()) {  // Should never happen
      assert(0);
      i++;
      it++;
      continue;
    ...
    if (intersections.size() == 2 &&
        intersections[0] == intersections[1]) {  // Only grazed
      i++;
      it++;
      continue;
    }
    ...
    UINT addedStroke =
        vi->getStrokeCount() - (oldStrokeSize - 1);  // -1 for the removed one
...
void EraserTool::erase(TVectorImageP vi, TRectD &rect) {
  ...
  TXshSimpleLevel *level =
      TTool::getApplication()->getCurrentLevel()->getSimpleLevel();
  // Release the previous undo if it exists
  if (m_undo) {
    delete m_undo;
    m_undo = nullptr;
  }
...
void EraserTool::multiEraseRect(TFrameId firstFrameId, TFrameId lastFrameId,
  ...
  for (int i = 0; i < m; ++i) {
    ...
    // m_level->setFrame(fid, img); // Necessary: if getFrame uncompressed a
    // compressed image, without setFrame the modifications made here would be
    // lost.

    // Set fid as current to notify the image change
    TTool::Application *app = TTool::getApplication();
    if (app) {
      if (app->getCurrentFrame()->isEditingScene())
...
void EraserTool::onImageChanged() {
  ...
  if (!xshl || m_level.getPointer() != xshl ||
    ...
    m_firstFrameSelected = false;  // If I passed to state 1 and return to the
...
//! The polyline is closed and erasing begins.
/*! A stroke representing the drawn polyline is created.
  If "frame range" erase method is selected, \b multiEraserRegion is called,
  otherwise \b eraseRegion is called*/
void EraserTool::leftButtonDoubleClick(const TPointD &pos,
  ...
  if (m_eraseType.getValue() == POLYLINE_ERASE) {
    ...
    std::unique_ptr<TStroke> stroke(new TStroke(strokePoints));
    assert(stroke->getPoint(0) == stroke->getPoint(1));

    if (m_multi.getValue()) {
      multiErase(stroke.get(), e, &EraserTool::eraseRegion);
    } else {
      eraseRegion(vi, stroke.get());
      m_active = false;
      notifyImageChanged();
    }
    // The unique_ptr will be automatically released when it goes out of scope
    invalidate();
  }
}
...
//! \b pos is added to \b m_track and the first piece of the freehand is drawn.
...
//! \b pos is added to \b m_track and another piece of the freehand is drawn.
...
//! The freehand is closed (last point added to m_track) and a stroke
//! representing the freehand is created.
...
//  m_viewer->enableRedraw(true);
  ...
  // Release the previous stroke if it exists
  if (m_stroke) {
    delete m_stroke;
  }
  m_stroke = m_track.makeStroke(error);
...
//! A point is added to the m_polyline vector.
...
//! Adds the last pos to \b m_polyline and closes the polyline (adds
//! \b m_polyline.front() to the end of the vector)
...
//! Erases strokes present in \b vi and contained in the region delimited by \b
//! stroke.
/*!Strokes to be erased are searched with the necessary checks on parameters \b
   m_invertOption and \b m_selective.
        If a stroke should be erased, it is inserted in \b eraseStrokes.
        All strokes are erased at the end.*/
...
//! doErase is called on frames between \b firstFrameId and \b lastFrameId.
/*!Vector \b fids is loaded with TFrameIds of images on which to perform
 * erasure.*/
void EraserTool::doMultiErase(TFrameId &firstFrameId, TFrameId &lastFrameId,
  ...
  std::unique_ptr<TStroke> first(new TStroke());
  std::unique_ptr<TStroke> last(new TStroke());
  *first = *firstStroke;
  *last  = *lastStroke;

  TVectorImageP firstImage = new TVectorImage();
  TVectorImageP lastImage  = new TVectorImage();
  firstImage->addStroke(first.release());  // Transfere posse para a imagem
  lastImage->addStroke(last.release());    // Transfere posse para a imagem
  ...
  for (int i = 0; i < m; ++i) {
    ...
    double t = m > 1 ? (double)i / (double)(m - 1) : 0.5;
    t        = TInbetween::interpolation(t, algorithm);
    // Set the fid as current to notify the image change
    if (app) {
      if (app->getCurrentFrame()->isEditingScene())
...
//! \b eraseRegion is called for the correct frame in "Frame Range" mode.
/*!In cases where \b t is different from zero and one, a new \b TVectorImageP is
   generated by calling \b TInbetween.
        The new image contains the stroke to give to eraseRegion. \b fid is the
   TFrameId of the image on which
        erasure should be performed.*/
void EraserTool::doErase(double t, const TXshSimpleLevelP &sl,
  ...
  if (t == 0)
    (this->*eraseFunction)(img, firstImage->getStroke(0));
  else if (t == 1)
    (this->*eraseFunction)(img, lastImage->getStroke(0));
  else {
    assert(firstImage->getStrokeCount() == 1);
    assert(lastImage->getStrokeCount() == 1);
    TVectorImageP vi = TInbetween(firstImage, lastImage).tween(t);
    assert(vi->getStrokeCount() == 1);
    (this->*eraseFunction)(img, vi->getStroke(0));
  }
}

//-----------------------------------------------------------------------------

//! Performs erasure in "Frame range" mode.
/*! If the first frame has already been selected, call \b doMultiErase;
 otherwise \b m_firstStroke is initialized.*/
