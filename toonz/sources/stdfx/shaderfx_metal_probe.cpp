...
void printUsage() {
  std::cerr
      << "usage: shaderfx_metal_probe [--shader-folder DIR] [--write-pam FILE]\n"
         "                            [--compare-pam FILE] [--write-diff-pam FILE]\n"
         "                            [--tolerance N]\n";
}
...
TRaster32P makeDiffRaster(const TRaster32P &actual, const TRaster32P &expected) {
  ...
  for (int y = 0; y < actual->getLy(); ++y) {
    ...
    for (int x = 0; x < actual->getLx(); ++x) {
      diffRow[x] = TPixel32(
          std::abs(actualRow[x].r - expectedRow[x].r),
...
int main(int argc, char *argv[]) {
  ...
  if (!renderSunflareShaderFxForProbe(tile, 1.0, settings))
  ...
  std::cout << "shaderfx_metal_probe: ok backend=" << activeBackendName();
  if (TGraphics::activeBackendType() == TGraphics::BackendType::Metal)
