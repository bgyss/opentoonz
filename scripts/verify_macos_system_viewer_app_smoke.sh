...
artifact_dir="${1:-${OPENTOONZ_SYSTEM_VIEWER_APP_SMOKE_DIR:-${TMPDIR:-/tmp}/opentoonz-system-viewer-app-smoke}}"
...
OPENTOONZ_GRAPHICS_SMOKE_SCREENSHOT="${OPENTOONZ_GRAPHICS_SMOKE_SCREENSHOT:-1}" \
  "$repo_root/scripts/macos/graphics-app-smoke.sh" "$artifact_dir"

"$repo_root/scripts/verify_graphics_app_smoke_artifacts.sh" \
  "$artifact_dir" --require-screenshot

echo "verify-macos-system-viewer-app-smoke: ok artifacts=$artifact_dir"
