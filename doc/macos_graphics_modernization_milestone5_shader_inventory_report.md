...
`glLineStipple(...)`, and `GL_LINE_STRIP` drawing to `DrawList2D` color-line
...
The selection overlay checkpoint routes the polyline selection start marker and
editable-selection center crosshair circles/segments through `DrawList2D`
color-circle/color-line commands. Rectangle, freehand, deformation, and drag
handle drawing remain on their existing paths for separate migrations.
The fill preview checkpoint routes the area-fill polyline preview and drag-line
...
The skeleton joint state checkpoint removes redundant OpenGL color-state calls
...
- `SHADER_caustics` and `SHADER_starsky` remain shader-specific parity risks.
  On local Apple M1 Max hardware they have passed the OpenGL/Metal comparison
  used by `scripts/graphics_shaderfx_compare.sh`, but GitHub-hosted Apple
  Paravirtual runners produced materially different procedural noise fields.
  The CI script therefore keeps them in the smoke/artifact set by default and
  allows parity investigation with `SHADERFX_CAUSTICS_COMPARE=1` and
  `SHADERFX_STARSKY_COMPARE=1`.
...
`fireball` has the same CI policy: it runs through direct, detached renderer,
`ToonzScene`/`buildSceneFx(...)`, and saved-and-reloaded `.tnz` coverage, but
its OpenGL-vs-Metal pixel parity check is opt-in through
`SHADERFX_FIREBALL_COMPARE=1`. When enabled, `fireball` uses
`SHADERFX_FIREBALL_COMPARE_TOLERANCE=32` by default because its procedural
noise uses transcendental functions where OpenGL GLSL and Metal Shading
Language rounding can diverge at isolated threshold pixels. This keeps strict
tolerances for the stable migrated shaders while still failing broad color,
...
Checked 281 Mach-O files for arm64.
tgraphics_metal_probe: ok on Apple M1 Max
tnztools/libtnztools.dylib linked after selection overlay migration
OpenToonz linked after selection overlay migration
OpenToonz graphics API inventory: all graphics markers files=106 matches=2233; fixed-function drawing files=63 matches=1458
WITH_GRAPHICS_METAL:BOOL=OFF
WITH_GRAPHICS_METAL:BOOL=ON
Checked 281 Mach-O files for arm64.
tgraphics_metal_probe: ok on Apple M1 Max
...
Checked 281 Mach-O files for arm64.
tgraphics_metal_probe: ok on Apple M1 Max
tnztools/libtnztools.dylib linked after skeleton redundant color-state cleanup
