...
## Post-Migration Rust Successor

The Rust migration is a successor program, not an additional Qt 6 completion
requirement. After this completion contract and the separate Metal/Vulkan
renderer migration are both accepted, begin with:

1. `doc/rust_migration_study.md` for the architecture and framework survey;
2. `doc/rust_porting_workflow.md` for the incremental replacement procedure;
3. `doc/rust_port_codex_goal_prompt.md` for the first bounded bootstrap goal.

That program keeps Qt 6, uses the official Qt Bridge for Rust only where its
Qt Quick/QML scope and maturity fit, uses CXX/CXX-Qt for the existing Widgets
transition, and requires a C++ oracle before production behavior moves.
